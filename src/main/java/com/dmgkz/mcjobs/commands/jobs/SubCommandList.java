...
public class SubCommandList {
    public static void command(Player p) {
        ...
        if(!(p.hasPermission("mcjobs.jobs.list") || p.hasPermission("mcjobs.jobs.all")) && McJobs.getPlugin().getConfig().getBoolean("advanced.usePerms", true)) {
            str = ChatColor.RED + McJobs.getPlugin().getLanguage().getJobCommand("permission", p.getUniqueId()).addVariables("", p.getName(), "");
        ...
        if(!Bukkit.getVersion().toLowerCase().contains("spigot") || !Bukkit.getPluginManager().isPluginEnabled("WorldEdit")) {
            ...
            iJob = SpigotBuilds.getPlayerHasJobs(p);
