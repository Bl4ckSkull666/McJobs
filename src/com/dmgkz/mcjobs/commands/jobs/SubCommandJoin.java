...
public class SubCommandJoin {
    public static void command(Player p, String[] a) {
        ...
        if(PlayerData.hasJob(p.getUniqueId(), job)) {
            p.sendMessage(ChatColor.RED + McJobs.getPlugin().getLanguage().getJobJoin("have", p.getUniqueId()).addVariables(pJob, p.getName(), ""));
            return;
        }
        ...
        if(!(p.hasPermission("mcjobs.jobsavail." + job) || p.hasPermission("mcjobs.jobsavail.all")) && McJobs.getPlugin().getConfig().getBoolean("advanced.usePerms")) {
            p.sendMessage(ChatColor.GRAY + McJobs.getPlugin().getLanguage().getJobJoin("jobperm", p.getUniqueId()).addVariables(pJob, p.getName(), ""));
        ...
        if(!PlayerData.isJoinable(p.getUniqueId(), job)) {
            ...
            String str = ChatColor.GRAY + McJobs.getPlugin().getLanguage().getJobJoin("timer", p.getUniqueId()).addVariables(pJob, p.getName(), timed);
        ...
        if(PlayerData.getJobCount(p.getUniqueId()) < PlayerData.getAllowedJobCount(p.getUniqueId())) {
            ...
            p.sendMessage(ChatColor.GRAY + McJobs.getPlugin().getLanguage().getJobJoin("join", p.getUniqueId()).addVariables(pJob, p.getName(), ""));

            McJobsEventJobChange event = new McJobsEventJobChange(p, job, true, false);
            Bukkit.getServer().getPluginManager().callEvent(event);
        } else {
            p.sendMessage(ChatColor.GRAY + McJobs.getPlugin().getLanguage().getJobJoin("toomany", p.getUniqueId()).addVariables(pJob, p.getName(), ""));
            p.sendMessage(ChatColor.YELLOW + McJobs.getPlugin().getLanguage().getJobJoin("command", p.getUniqueId()).addVariables(pJob, p.getName(), ""));
            return;
        }
